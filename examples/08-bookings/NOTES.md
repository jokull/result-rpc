...
| --- | --- | --- |
| `orders.list` row wrapper `{ order, bookedBy, lineItems }` | one-off `wire.object` | The *pairing* is query shape, not a fact about anything. Inline objects collect no identity — deliberately. |
...
| `hotels.reviews` page `{ rows, hasMore }` | one-off | Page composition and the `hasMore` sentinel are query-relative by definition (they describe *this page of this feed*). |
...
| `hotels.reviewStats` `{ count, averageRating }` | one-off | A `count()`/`avg()` aggregate over a table is a fact about the *query*, not about any row. Freshened by `.affects`, never by patching. |
| `tours.byId` / `tours.featured` rows | `TourContent.codec`, key `["id","locale"]` (derived) | The locale trap, closed structurally: `(t-fuji, en)` and `(t-fuji, ja)` are *different entities*. A single-field `id` key would smear an English title edit into the Japanese cache. |
...
8. **Mixed mutation blast radius** — with page 2 cached but collapsed,
   ...
   inactive cached page is invalidated *without* being fetched — it fetches
...
What the freshness in tests 2 and 7 actually costs to *declare*, counted in
real lines.
...
`models.ts` overall: 114 lines before → 142 after, while *adding* a fourth
...
`.affects` lines *in the contract, once*.
...
- `users.rename` reaching 4 surfaces across 2 *pages*:
  ...
  - **~20 lines** — or 3+ invalidations = one refetch *per cached page* per
    rename.
...
runtime patches *every query containing `user:u-kenji`* — it has no concept
...
- **The big one: nested `with` on SQLite emits `jsonb_*` functions**, which
  ...
  isolation but may be called only before *anything* in the process opens a
...
- **The enum mapping surfaced a real drift.** Round one's hand-written
  `TourContent` declared `locale` as a literal union, but the *table* said
