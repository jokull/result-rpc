...
| `08-bookings` | travel bookings on Drizzle 1.0 | the ground-truth proof: real SQL, four-level tree with models only at keyed nodes, composite-key locale content, query-relative aggregates that never smear, derived summaries via `.affects`, record-key `touch`, models derived via `modelFromDrizzle`, `tryDb` constraint-collapse, cross-page rename at one request — every claim counter-pinned; `NOTES.md` is the shape-decision table + cost ledger |
