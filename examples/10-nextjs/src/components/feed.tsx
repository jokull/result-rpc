 ...
 * "success" immediately, no skeleton flash. The skeleton only appears on
 * a client-side navigation nothing prefetched.
 */
import { useState } from "react";
import { useResultMutation, useResultPaginatedQuery, useResultQuery } from "result-rpc/react";
import Link from "next/link";
import { client } from "../client";
import type { SpotRow } from "../models";
import { FeedSkeleton, StatsSkeleton } from "./skeleton";
...
const SpotCard = ({ spot }: { spot: SpotRow }) => (
  <li className="card">
    <div className="card-head">
      <Link href={`/spots/${spot.id}`} className="card-title">
        {spot.name}
      </Link>
...
export const Feed = () => {
  // staleTime trusts the server prefetch for a minute: first mount makes
  // ZERO client requests — the whole point of RSC prefetching.
  ...
  return (
    <>
      ...
      <p className="meta">
        {feed.rows.length} spots loaded · {feed.pageCount} page{feed.pageCount === 1 ? "" : "s"}
      </p>
    </>
  );
};
