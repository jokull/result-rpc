...
const isGraphInput = (value: unknown, seen = new WeakSet<object>()): value is GraphInput => {
  if (value === null || typeof value !== "object") return false;
  if (seen.has(value)) return true;
  seen.add(value);
  if (!("sequence" in value) || typeof value.sequence !== "bigint") return false;
  if (!("labels" in value) || !(value.labels instanceof Map)) return false;
  for (const [key, label] of value.labels) {
    if (typeof key !== "string" || typeof label !== "string") return false;
  }
  return !("self" in value) || value.self === undefined || isGraphInput(value.self, seen);
};
const graph = r
  .procedure()
...
describe("reactive query runtime", () => {
  ...
  test("rejects procedures from a different client instance", () => {
    ...
    const otherClient = createFixtureClient({ router, transport });
    const runtime = createQueryRuntime({ client });
    ...
    const terminal = await new Promise<ReturnType<typeof subscription.getCurrentState>>(
      (resolve) => {
        const unsubscribe = subscription.subscribe(() => {
          ...
          if (state.connection === "closed" && !state.result?.isOk()) {
            unsubscribe();
            resolve(state);
          }
        });
...
describe("offline anti-thrash", () => {
  ...
  test("a mutation attempted offline fails loud with client/offline, exactly once", async () => {
    ...
    try {
      ...
      if (!result.isOk()) expect(result.error._tag).toBe("client/offline");
      expect(attempts).toBe(1);
      mutation.destroy();
    } finally {
      globalThis.dispatchEvent(new Event("online"));
...
describe("mutation retry safety", () => {
  test("a mutation NEVER retries an ambiguous mid-flight failure by default", async () => {
    ...
    expect(result.isOk()).toBe(false);
    if (!result.isOk()) expect(result.error._tag).toBe("client/network-failure");
    expect(attempts).toBe(1);
    mutation.destroy();
    runtime.clear();
  });
