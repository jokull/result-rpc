...
export type ClientProcedureCapability =
  | ClientUnaryTypes
...
export interface ProcedureClientTypes<
  TProcedure extends ClientProcedure,
...
export type ClientProcedurePagination<TProcedureClient> =
  ClientProcedureTypes<TProcedureClient>["capability"] extends infer TCapability
    ? TCapability extends ClientPaginationTypes<unknown, unknown, unknown>
      ? TCapability
      : never
    : never;
...
export type ProcedureClientTypesFor<
  TProcedure,
  TBoundaryError extends AnyPublicTaggedError,
> = TProcedure extends ClientProcedure ? ProcedureClientTypes<TProcedure, TBoundaryError> : never;

export type ProcedureClientFromTypes<
  TTypes extends AnyProcedureClientTypes,
  TOptions,
  TSubscriptionMode extends "managed" | "iterable",
> = TTypes["kind"] extends "subscription"
  ? ((
      ...args: ClientCallArgs<TTypes["input"], TOptions>
    ) => SubscriptionResult<TSubscriptionMode, TTypes["output"], TTypes["error"]>) & {
      readonly $kind: TTypes["kind"];
    } & ProcedureClientTypeCarrier<TTypes>
  : ((
      ...args: ClientCallArgs<TTypes["input"], TOptions>
    ) => Promise<Result<TTypes["output"], TTypes["error"]>>) & {
      readonly $kind: TTypes["kind"];
    } & ProcedureClientTypeCarrier<TTypes>;
...
export type ClientRecordError<TRecord> = {
  readonly [TKey in keyof TRecord]: TRecord[TKey] extends ClientProcedure
    ? ErrorUnion<ProcedureTypesOf<TRecord[TKey]>["definitions"]>
    : TRecord[TKey] extends ClientRouterRecord
      ? ClientRecordError<TRecord[TKey]>
      : never;
}[keyof TRecord];

export type ClientRouterRecordOf<TRouter> = Extract<
  RouterRecordOf<TRouter>,
  RouterRecord | ContractRouterRecord
>;
...
export type BaseClientOf<
  ...
  TOptions,
  TSubscriptionMode extends "managed" | "iterable",
> = ClientRecord<ClientRouterRecordOf<TRouter>, TBoundaryError, TOptions, TSubscriptionMode> & {
...
const clientErrorRegistry = <E extends AnyTaggedError>(
  source: readonly AnyPublicErrorDefinition[],
...
export const createClientErrorRegistry = <E extends AnyPublicTaggedError>(
  ...
  clientErrorRegistry([...router.errors.values(), ...boundaryDefinitions]);

/** Runtime counterpart of one generated callable's exact error union. */
export const createProcedureClientErrorRegistry = <E extends AnyTaggedError>(
  procedure: ClientProcedure,
  boundaryDefinitions: readonly AnyPublicErrorDefinition[],
): ClientErrorRegistry<E> =>
  clientErrorRegistry([...Object.values(procedure._def.definitions), ...boundaryDefinitions]);
